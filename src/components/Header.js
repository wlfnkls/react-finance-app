    ...
    <header className="bg-white shadow-lg sticky top-0">
      ...
      {formVisible &&
        <form className="flex flex-col mt-4" onSubmit={onSubmit}>
          <input value={text} onChange={(e) => setText(e.target.value)} className="bg-gray-100 rounded p-2 my-2 focus:outline-none" type="text" placeholder="Position" />
          <input value={value} onChange={(e) => setValue(e.target.value)} className="bg-gray-100 rounded p-2 mb-4 focus:outline-none" type="text" placeholder="Wert" />
          <input className="bg-green-500 text-white rounded px-6 py-2 shadow hover:shadow-md hover:bg-green-600 active:transform active:scale-95 cursor-pointer" type="submit" value="Add" />
        </form>
      }
    </header>
  )
